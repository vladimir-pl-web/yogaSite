...
function modal() {
  ...
  descriptionBtn.forEach(function (e) {
    /// модальное окно привяжем к кнопкам узнать больше
  ...
  more.addEventListener('click', function () {
    /// модальное окно привяжем к большшой кнопке
    overlay.style.display = 'block';
    this.classList.add('more-splash');
    document.body.style.overflow = 'hidden';
  });
...
function slider() {
  ...
  function showSlides(n) {
    // сделаем карусель
    if (n > slides.length) {
      // если n, то есть номеh слайда больше длины слайдера
    ...
    }); // убираем активный класс с точек
...
function tabs() {
  ...
  var tabContent = document.querySelectorAll(".info-tabcontent"); /// скрываем содержимое табов с помощью замены классов,
  ...
  hideTabContent(1); /// а тут показываем содержимое. если контент содержит класс хайд
...
function timer() {
  /// Таймер
  var deadline = '2019-08-01'; // Узнаем остаток между сейчас и дедлайном в милисекундах

  var timeRemaining = function timeRemaining(endtime) {
    var t = Date.parse(endtime) - Date.parse(new Date()),
        //(с помощью parse)
    seconds = Math.floor(t / 1000 % 60),
        // округляем до целого ( Math.floor), получаем секунды и остаток от деления на 60
    minutes = Math.floor(t / 1000 / 60 % 60),
        // округляем-секунды-минуты-остаток от деления на 60
  ...
  var setTime = function setTime(id, endtime) {
    // получим нужные переменные
    var timer = document.getElementById(id),
        hours = timer.querySelector('.hours'),
        minutes = timer.querySelector('.minutes'),
        seconds = timer.querySelector('.seconds'),
        timeInterval = setInterval(timeUpdate, 1000); /// Сделаем функцию обновляющую таймер

    function timeUpdate() {
      // передадим в переменную объект что вернула функция timeRemaining 
      var r = timeRemaining(endtime);
...
window.addEventListener("DOMContentLoaded", function () {
  ...
  var calc = __webpack_require__(/*! ./modules/calc.js */ "./src/js/modules/calc.js"),

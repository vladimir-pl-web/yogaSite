...
info.addEventListener('click', e => {
  let target = e.target;
  if(target && target.classList.contains('info-header-tab')) { // если таргет и его классы содержат info-header-tab
    for ( let i = 0; i < tab.length; i++) {// то перебираем кнопки
      if(target == tab[i]) {// и если таргет равен индексу таба 
        console.log(tab[i]);
